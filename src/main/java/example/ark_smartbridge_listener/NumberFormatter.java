...
public class NumberFormatter {
    ...
    public String formatNumber(BigDecimal value) {
        if (value.compareTo(new BigDecimal("1000000")) > 0) {
            return value.divide(new BigDecimal("1000000"), 2, BigDecimal.ROUND_DOWN).toPlainString() + "M";

        } else if (value.compareTo(new BigDecimal("1000")) > 0) {
            return value.divide(new BigDecimal("1000"), 2, BigDecimal.ROUND_DOWN).toPlainString() + "k";
        } else {
